...
test("App fetches shows data from the api and renders it", async () => {
    //Arrange
    mockFetchShows.mockResolvedValueOnce(shows);
    const {findByText, getByText, queryByText, getAllByTestId, getByTestId} = render(<App />);
    //Act
    ...
    const selectSeasonButton =  getByText(/Select a season/i)
     ...
     waitFor(() => {
        ...
        expect(getByTestId(/show/i)).toHaveLength(1);
    })
